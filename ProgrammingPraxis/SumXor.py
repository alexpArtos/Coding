...
def generate_all_values_to_list(a, free_bits):
    ...
    '''
    values = [a]
    free_bits_weights = compute_free_bits_weights_list(free_bits)
    # for each bit 1 in the the xor mask
    # I only add all bits except the last one, because we only need the first half of pairs. The rest is complementary
    # Example:
    # s = 9, x = 5
    # fixed: 0x1x
    # first bit gives:
    ...
    for i in range(0, len(free_bits)-1):
        # copy each value in the list
        newvalues = []
        for value in values:
            # while setting this bit to 1
            newvalue = value | free_bits_weights[i]
...
def generate_and_print_all_pairs_just_in_time(a, free_bits):
    ...
    print_pair(value, x)
    count = 1
    # only outputting half the pairs, since they come in pairs anyway
...
def compute_free_bits_weights_list(free_bits):
    '''
        Instead of obtaining the position j that changes between two consecutive Gray codewords,
        it is easier to obtain 1 << j.
        This method builds a dictionary that associates these change values with the value of the
